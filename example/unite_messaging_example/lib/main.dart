...
import 'package:unite_messaging_example/models/message.dart';
import 'package:unite_messaging_example/dialogs/send_message.dart';
...
class _MyAppState extends State<MyApp> {
  ...
  // String server = 'grpc://10.0.2.2:6061';
  // unite.Client client;
  ...
  Future _connect() async {
    ...
    final topicFilter =
        unite.TopicFilter("teams.alpha.*", client.messageStream);
  ...
  unite.ConnectionLostHandler onConnectionLost = (unite.Connection client) {
    print("Connection lost: \n");
  };

  void _onMessage(List<unite.Message> e) {
    print(e.length);
    print("TOPIC: ${e[0].topic}\n");
    print("MSG: ${e[0].payload}\n");
    print("Qos: ${e[0].qos}\n");

    final String message = e[0].payload;
    ...
    setState(() {
      messages.add(Message(
          topic: e[0].topic, message: message, qos: unite.Qos.values[0]));
      try {
  ...
  void _subscribeToTopic(String topic) {
    setState(() {
      if (topics.add(topic.trim())) {
        ...
        client.subscribe(topic, qos: unite.Qos.atLeastOnce);
      }
    });
  }
