...
abstract class Topic {
  ...
  static void validateMinLength(Topic topicInstance) {
    if (topicInstance.topic.isEmpty) {
      throw Exception(
          'unite_messaging::Topic: Topic must contain at least one character');
    }
  }
  ...
  static void validateMaxLenth(Topic topicInstance) {
    if (topicInstance.topic.length > maxTopicLength) {
      throw Exception(
          'unite_messaging::Topic: The length of topic ${topicInstance.topic.length} is longer than the max Topic length allowed $maxTopicLength');
    }
  }
  ...
  static void validateMaxDepth(Topic topicInstance) {
    if (topicInstance.topicParts.length > maxTopicDepth) {
      throw Exception(
          'unite_messaging::Topic: The depath of topic ${topicInstance.topicParts.length} is longer than the max Topic length allowed $maxTopicDepth');
    }
  }
  ...
  static void validateMultiWildcard(Topic topicInstance) {
    if (topicInstance.topic.contains(multiWildcardSymbol) &&
      ...
      throw Exception(
          'unite_messaging::Topic: The topic multiwild ... can only be present at the end of a topic');
    }
  }
  ...
  static void validateTopicParts(Topic topicInstance) {
    ...
    if (invalidPart) {
      throw Exception(
          'unite_messaging::Topic: Topic part contains a wildcard but is more than one character long');
    }
  }
}
